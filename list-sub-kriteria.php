...
if (isset($_POST['tambah'])) :
	...
	// if(empty($errors)):
	// 	$simpan = mysqli_query($koneksi,"INSERT INTO sub_kriteria (id_sub_kriteria, id_kriteria, nama, nilai, tahap, tahun) VALUES ('', '$id_kriteria', '$nama', '$nilai', '$tahap', '$tahun')");
	...
	if (empty($errors)) {
		try {
			$stmt = $koneksi->prepare("INSERT INTO sub_kriteria (id_kriteria, nama, nilai, tahap, tahun) VALUES (:id_kriteria, :nama, :nilai, :tahap, :tahun)");
			...
			$sts[] = 'Terjadi kesalahan: ' . $e->getMessage();
		}
	}
endif;
...
if (isset($_POST['edit'])) :
	...
	if (empty($errors)) {
		try {
			$stmt = $koneksi->prepare("UPDATE sub_kriteria SET nama = :nama, nilai = :nilai WHERE id_kriteria = :id_kriteria AND id_sub_kriteria = :id_sub_kriteria");
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
	...
	<a onclick="return confirm ('Apakah anda yakin untuk meghapus semua data ini?')" href="hapus-all-sub-kriteria.php" class="btn btn-danger"> <i class="fa fa-trash"></i> Hapus Semua Data </a>
</div>
				...
				<div class="table-responsive">
					<table class="table table-bordered" width="100%" cellspacing="0">
						...
						<tbody>
							...
							$stmt = $koneksi->prepare("SELECT * FROM sub_kriteria WHERE id_kriteria = :id_kriteria ORDER BY nilai DESC");
							$stmt->bindParam(':id_kriteria', $id_kriteria);
							$stmt->execute();
